// *******************String Methods*******************
// *******************length*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)

// let txtLength = txt.length
// console.log(`After : ${txtMethod}`)

// *******************toString()*******************

// let num = 78

// console.log(num)
// let txtMethod = num.toString()
// console.log(txtMethod)


// *******************toLowerCase()*******************

// let txt = "Lorem ipsum DOLOR sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.toLowerCase()
// console.log(`After : ${txtMethod}`)

// *******************toUpperCase()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.toUpperCase()
// console.log(`After : ${txtMethod}`)

// *******************includes()*******************

// let txt = "Lorem ipsum dolor 900 sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.includes("ipsum")
// console.log(`After : ${txtMethod}`)

// *******************startWith()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.startsWith("Lorem")
// console.log(`After : ${txtMethod}`)

// *******************endsWith()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.endsWith("elit.")
// console.log(`After : ${txtMethod}`)

// *******************search()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.search("elit.")
// console.log(`After : ${txtMethod}`)

// *******************match()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.match(/sit/g)
// let txtMethod = txt.match("sit")
// console.log(`After : ${txtMethod}`)




// *******************indexOf()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.indexOf("ip")
// console.log(`After : ${txtMethod}`)

// *******************lastIndexOf()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.lastIndexOf("ip")
// console.log(`After : ${txtMethod}`)

// *******************replace()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.replace(/ip/g, "it")
// // let txtMethod = txt.replace("ip", "it")
// console.log(`After : ${txtMethod}`)

// *******************trim()*******************

// let txt = "        Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.trim()
// console.log(`After : ${txtMethod}`)

// *******************charAt()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.charAt(6)
// console.log(`After : ${txtMethod}`)

// *******************charCodeAt()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.charCodeAt(6)
// console.log(`After : ${txtMethod}`)

// *******************fromCharCode()*******************

// let txtMethod = String.fromCharCode(65)
// console.log(`${txtMethod}`)

// *******************concat()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.concat("Hello world", " hye there")
// console.log(`After : ${txtMethod}`)

// *******************split()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.split("ipsum")
// console.log(`After : ${txtMethod}`)

// *******************repeat()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.repeat(2)
// console.log(`After : ${txtMethod}`)

// *******************slice()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.slice(2,6)
// console.log(`After : ${txtMethod}`)

// *******************substr()*******************

// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.substr(2,6)  //second parameter use for how many characters we want to pick
// console.log(`After : ${txtMethod}`)

// *******************substring()*******************
// 
// let txt = "Lorem ipsum dolor sit amet consectetur adipisicing elit."

// console.log(`Before : ${txt}`)
// let txtMethod = txt.substring(2,6)  //second parameter use like a slice but it needs + 1 end position
// console.log(`After : ${txtMethod}`)



